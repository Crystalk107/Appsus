...
//     return gBooks.filter(book=> book.title.includes(filterBy.title) && book.listPrice.amount <= filterBy.amount)
//  }
let gBooks = storageService.load('books') || createBooks();

function getBooks(filterBy) {
  const books = (!filterBy) ?
    [...gBooks]
    : gBooks.filter(book => book.title.includes(filterBy.title)
      && book.listPrice.amount <= filterBy.amount);
...
function getBooksFromGoogle(bookName) {
  let prmAns = axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${bookName}`)
  return prmAns.then((response) => {
    let { data } = response;
    let searchResults = data.items.slice(0, 10)
...
function saveBook(bookName){
  let filteredBook = getBooksFromGoogle(bookName);
  console.log (filteredBook)
  const book = new Book(filteredBook.id,
     filteredBook.volumeInfo.title, 
     filteredBook.volumeInfo.subtitle, 
     filteredBook.volumeInfo.authors, 
     +filteredBook.volumeInfo.publishedDate.substring(0,3), 
     filteredBook.volumeInfo.description, 
     filteredBook.volumeInfo.pageCount, 
     filteredBook.volumeInfo.categories, 
     filteredBook.volumeInfo.imageLinks.thumbnail,
      filteredBook.volumeInfo.language, 
      filteredBook.saleInfo.listPrice.amount, 
      filteredBook.saleInfo.listPrice.currencyCode, 
      (filteredBook.saleInfo.saleability === 'FOR_SALE') ? true : false
      )
  gBooks =  [book, ...gBooks];
...
function getPageCountNotif(book) {
...
storageService.store('books', booksData)
return booksData;
}